...
class FieldMapping:
    def __init__(
        ...
        mappings : Union[list, tuple], optional
            ...
            (optional) key.
        disable_indexing : bool, optional
            Specifies whether indexing of items should be disabled. If set to
            True (which disabled indexing), 'source_field' duplicates are
            allowed but functionality of this class is severely reduced.
        ...
        self.__target_types = []
        self.__indexing_enabled = not disable_indexing
        if self.__indexing_enabled:
            self.__indices = {}
    ...
    def add(
        ...
        source_field_len = len(self.__source_fields)
        ...
        if self.__indexing_enabled:
            ...
            self.__indices.update({source_field: source_field_len})

        self.__source_fields = list(self.__source_fields[:source_field_len]) + [
            source_field
        ]
        self.__target_fields = list(self.__target_fields[:source_field_len]) + [
            target_field
        ]
        if target_type:
            self.__target_types = list(self.__target_types[:source_field_len]) + [
                target_type
            ]
    ...
    def remove(
        ...
        if not self.__indexing_enabled:
            raise NotImplementedError(
                "This function is unavailable because 'disable_indexing' was set to True"
            )
