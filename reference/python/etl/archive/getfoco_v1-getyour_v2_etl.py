...
def hash_address(address_dict: dict) -> str:
    ...
    concatVals = "".join(
        [
            ...
            if key in address_dict.keys() and isinstance(address_dict[key], str)
...
def update_current_users(global_objects: dict) -> None:
    ...
    cursorNew.execute(
        "update public.app_address set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_household set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_householdmembers set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_householdhist set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_iqprogram set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_eligibilityprogram set user_id=user_id+{cradd} where user_id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
    cursorNew.execute(
        "update public.app_user set id=id+{cradd} where id<{cradd}".format(
            cradd=global_objects["current_user_offset"]
        )
    )
...
def port_user(global_objects: dict) -> None:
    ...
    if len(userList) > 0:
        cursorNew.execute(
            """insert into public.app_user ("id", "password", "last_login", "is_superuser", "is_staff", "is_active", "date_joined", "email", "first_name", "last_name", "phone_number", "has_viewed_dashboard", "is_archived", "is_updated")
                          VALUES {}""".format(", ".join(["%s"] * len(userList))),
            userList,
        )
...
def port_address(global_objects: dict) -> None:
    ...
    cursorOld.execute(
        """SELECT {fd}, "created", "modified", "isInGMA", "isInGMA", "hasConnexion", "is_verified"
        from public.application_addresses
        order by "created" asc""".format(
            fd=", ".join(
                [
                    f'UPPER("{x[0]}")' if x[0] != "zipCode" else f'"{x[0]}"'
                    for x in fieldConversionList
                ]
            ),
        )
    )
    ...
    if len(addressList) > 0:
        ...
        for addritm in outAddressList:
            ...
            cursorNew.execute(
                """insert into public.app_addressrd ("{fd}", "created_at", "modified_at", "is_in_gma", "is_city_covered", "has_connexion", "is_verified", "address_sha1")
                              VALUES ({vl})
        ...
        for usritm in userAddressList:
            ...
            cursorNew.execute(
                """SELECT "id" FROM public.app_addressrd WHERE "address_sha1"=%s""",
                (hash_address(addressDict),),
            )
            idVal = cursorNew.fetchone()[0]

            cursorNew.execute(
                """INSERT INTO public.app_address ("created_at", "modified_at", "user_id", "eligibility_address_id", "mailing_address_id", "is_updated")
                              VALUES ({})""".format(
...
def port_eligibility(global_objects: dict) -> None:
    ...
    cursorOld.execute(
        """SELECT "created", "modified", "user_id_id", False, (CASE WHEN "rent" NOT IN ('Rent', 'Own') THEN "rent" ELSE '' END), "dependents", "AmiRange_max", (CASE WHEN "GenericQualified"='ACTIVE' THEN true ELSE false END), (CASE WHEN "rent" IN ('Rent', 'Own') THEN LOWER("rent") ELSE '' END) FROM public.application_eligibility"""
    )
    eligList = cursorOld.fetchall()

    if len(eligList) > 0:
        cursorNew.execute(
            """insert into public.app_household ("created_at", "modified_at", "user_id", "is_updated", "duration_at_address", "number_persons_in_household", "income_as_fraction_of_ami", "is_income_verified", "rent_own")
                          VALUES {}""".format(", ".join(["%s"] * len(eligList))),
            eligList,
        )
...
def port_householdmembers(global_objects: dict) -> None:
    ...
    cursorOld.execute(
        """SELECT "created_at", "modified_at", "user_id", "household_info", false FROM public.application_moreinfo_rearch"""
    )
    membersList = cursorOld.fetchall()

    if len(membersList) > 0:
        cursorNew.execute(
            """insert into public.app_householdmembers ("created_at", "modified_at", "user_id", "household_info", "is_updated")
                          VALUES {}""".format(", ".join(["%s"] * len(membersList))),
            # Serialize the JSON in membersList
            [
                tuple(
                    [
                        json.dumps(y) if idx == len(x) - 2 else y
...
def port_iqprograms(global_objects: dict) -> None:
    ...
    def update_program(global_objects, program_name, old_field_name):
        ...
        if len(programList) > 0:
            cursorNew.execute(
                """INSERT INTO public.app_iqprogram ("applied_at", "enrolled_at", "user_id", "is_enrolled", "program_id")
                              VALUES {}""".format(", ".join(["%s"] * len(programList))),
                programList,
            )
...
def port_feedback(global_objects: dict) -> None:
    ...
    cursorOld.execute(
        """SELECT "id", "created", "modified", "feedbackComments", "starRating" FROM public.dashboard_feedback"""
    )
    feedbackList = cursorOld.fetchall()

    if len(feedbackList) > 0:
        cursorNew.execute(
            """insert into public.app_feedback ("id", "created", "modified", "feedback_comments", "star_rating")
                          VALUES {}""".format(", ".join(["%s"] * len(feedbackList))),
            feedbackList,
        )
...
def verify_transfer(global_objects: dict) -> None:
            ...
            for idx, usrid in enumerate(newIdList):
                ...
                cursorOld.execute(
                    """select {fd} from public.application_addresses where "user_id_id"={usr}""".format(
                        fd=", ".join(
                            [
                                f'UPPER("{x[0]}")'
                                if x[0] in ("address", "address2", "city", "state")
                                else f'"{x[0]}"'
                                for x in fieldConversionList
                            ]
                        ),
                ...
                if len(oldRec) > 0 or len(newRec) > 0:
                    ...
                    assert all(
                        [
                            True
                            if (y == True and (x == False or x is None))
                            or (y == False and x is None)
                            else y == x
                            for x, y in zip(oldRec[0][-3:], newRec[0][-3:])
                        ]
                    )
                ...
                fieldConversionList = [
                    ...
                    ('"modified"', "modified_at"),
                    (
                        """(CASE WHEN "rent" NOT IN ('Rent', 'Own') THEN "rent" ELSE '' END)""",
                        "duration_at_address",
                    ),
                    ('"dependents"', "number_persons_in_household"),
                    ('"AmiRange_max"', "income_as_fraction_of_ami"),
                    (
                        """(CASE WHEN "GenericQualified"='ACTIVE' THEN true ELSE false END)""",
                        "is_income_verified",
                    ),
                    (
                        """(CASE WHEN "rent" IN ('Rent', 'Own') THEN LOWER("rent") ELSE '' END)""",
                        "rent_own",
                    ),
                ]
                ...
                for program_name, old_field_name in programList:
                    fieldConversionList = [
                        ("created", "applied_at"),
                        (
                            """(CASE WHEN "{fdn}"='ACTIVE' THEN true ELSE false END)""".format(
                                fdn=old_field_name
                            ),
                            "is_enrolled",
                        ),
                ...
                for program_name, document_titles in documentsList:
                    cursorOld.execute(
                        """select "{fd}" from public.dashboard_form where "user_id_id"={usr} and ({dct}) order by "{crf}" """.format(
                            ...
                            usr=usrid,
                            dct=" or ".join(
                                [f""""document_title"='{x}'""" for x in document_titles]
                            ),
                        )
                    )
                    oldRec = cursorOld.fetchall()
    ...
    cursorOld.execute(
        """SELECT "id", "created", "modified", "feedbackComments", "starRating" FROM public.dashboard_feedback order by "id" asc"""
    )
    oldFeedbackList = cursorOld.fetchall()

    cursorNew.execute(
        """SELECT "id", "created", "modified", "feedback_comments", "star_rating" from public.app_feedback order by "id" asc"""
    )
...
def add_missing_records(global_objects: dict) -> None:
    ...
    # program selected)
    cursorNew.execute(
        "select distinct user_id from public.app_eligibilityprogram where user_id not in (select distinct user_id from public.app_eligibilityprogram where program_id=1) order by user_id"
    )
    ...
    cursorNew.execute(
        "select count(*) from public.app_eligibilityprogram where program_id=1 group by user_id"
    )
