...
def port_identification_path_reference(global_objects: dict) -> None:
    ...
    cursor.execute(
        """select "id" from public.app_eligibilityprogramrd where "program_name"='identification'"""
    )
    ...
    cursor.execute("""select max("modified_at") from public.app_householdmembers""")
            ...
            if dbOut is not None:
                ...
                for jsonidx, jsonitm in enumerate(memberDict["persons_in_household"]):
                    ...
                    if "identification_path" not in jsonitm.keys():
                        ...
                        memberDict["persons_in_household"][jsonidx][
                            "identification_path"
                        ] = None
    ...
    if finalTimestamp > initTimestamp:
        ...
        raise AssertionError(
            "The target table has been modified; the transaction has been rolled back and must be restarted."
        )
        ...
        for userid in userList:
            ...
            if dbOut is not None:
                ...
                for jsonidx, jsonitm in enumerate(memberDict["persons_in_household"]):
                    ...
                    if (
                        "identification_path" in jsonitm.keys()
                        and jsonitm["identification_path"] is None
                    ):
