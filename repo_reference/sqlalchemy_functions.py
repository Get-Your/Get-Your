...
class DBMetadata:
    def __init__(
        self,
        db_profile: str,
        tables_to_load: list | tuple = None,
    ):
        """
        ...
        try:
            ...
            # - MySQL/MariaDB must contain 'MySQL' in the 'driver' config key
            ...
            if "driver" in cred.config:
                if cred.config["driver"].lower() == "oracle":
                    ...
                    if "tns" in cred.config:
                    ...
                    elif "server" in cred.config and "hostname" in cred.config:
                        self.connection_url = URL.create(
                            "oracle+oracledb",
                            username=cred.config["user"],
                            password=cred.password(),
                        ...
                        raise AttributeError(
                            "Could not connect to Oracle DB. 'TNS' or 'server'/'hostname' designation is required in the db_profile ('DSN' alone is invalid for this script)",
                        )
                    ...
                    self.connection_url = URL.create(
                        ...
                        query={"driver": re.sub("{|}", "", cred.config["driver"])},
                    )

                elif "mysql" in cred.config["driver"].lower():
                    self.db_type = "mysql"

                    # Construct the MySQL/MariaDB connection string
                    self.connection_url = URL.create(
                        "mysql+pymysql",
                        username=cred.config["user"],
                        password=cred.password(),
                        host=cred.config["hostname"],
                        database=cred.config["default_schema"],
                        # Add the charset in the URL query parameters
                        query={"charset": "utf8mb4"},
                    )

                else:
                    raise TypeError(
                        "Database type not recognized (SQL Server, Oracle, or MySQL (which includes MariaDB) were expected by 'driver')",
                    )
...
class FieldMapping:
    ...
    def target_fields(self):
        """
        Return the target_fields as a dinctionary relative to source_fields.
    ...
    def add(
        ...
        target_type : str
            The datatype of the target field, if different than the source. This
            will be taken as the input (not coerced). The default is None,
            specifying that the source datatype will be used as the target datatype.
...
def upsert_via_merge(
    ...
    key_fields: list | tuple = None,
):
    """
    Upsert to the input table via MERGE from a cloned temporary table.
